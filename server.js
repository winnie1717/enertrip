...
app.post('/generate', async (req, res) => {
    ...
    try {
        ...
        const prompt = `
            ...
            【規則一：嚴格路徑結構 (Topology)】(必須嚴格遵守)
            1. ** 每個 Day 的結構**：
               [景點] -> [交通] -> [景點] -> ... -> [交通] -> [景點]
               *(注意： 每天的第一個結構必須是景點 例如:赤崁樓。)*
               *(注意： 旅遊景點中不須住宿 例如:台南酒店。)*

            2. **邊與節點交替**(必須嚴格遵守)：
               同一天中，兩個景點(Spot)之間必須插入一個交通(Transport)。
               *(注意：即使兩個景點重疊，也必須插入一個步行交通)*
               *(注意：今天的第一個景點 與 前一天的景點 不需要交通連接)*
            ...
            1. **SpotName (景點名稱)**：
               - 只能填寫真正的景點名字。
               - 景點名字禁止加上其他任何描述，禁止加上任何形容詞、動詞或各種名詞組合(例如 參訪台南美術館、赤嵌樓周邊美食、飯店周邊晚餐)。
               - 禁止在後面填括弧 () （）或是 & ，僅填寫文字。 
               - 一個景點名稱僅有一個景點，禁止填寫 景點A與景點B (例如 藍晒圖文創園區與海安路藝術街)作為景點名稱。
               - 禁止填寫活動，禁止填寫地區+動詞 (例如 台南市區休憩、台南市區自由活動)。 
               - 禁止填寫地區範圍 (例如 台南車站周邊、台南市郊區、赤嵌樓周邊美食、飯店周邊晚餐)。 
            ...
            【規則六：偏好的參考方式】
            1. **判斷疲勞偏好的高低**：
            - 所有偏好與疲勞預設皆為 5 ，使用者有調整才會記錄。
            - 因此紀錄中比 5 高，則為較高偏好、較高疲勞。
            - 因此紀錄中比 5 低，則為較低偏好、較低疲勞。
            - 根據比較高低程度進行調整。

            2. **檢查偏好清單**：
            - 加入景點前，優先檢查是否已存在偏好紀錄，若已存在紀錄，則考量下列 3-5 點決定是否加入行程。
            ...
            4. **低偏好移除**：
            - 低偏好移除，可替換成其他符合需求和偏好(可根據偏好推測)的景點。
